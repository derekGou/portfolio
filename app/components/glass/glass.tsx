...
export default function Glass({dark, children, className}: Props){
    ...
    return (
        <>
            <div className={`${myClass} ${className} overflow-hidden border-solid border shadow-2xl shadow-[#000c] h-fit w-fit flex items-center justify-center rounded-3xl backdrop-blur-sm`}>
                {children}
            </div>
        </>
    )
